          ...
          {result.validation_results && (
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(result.validation_results).map(
                  ([key, value]) => (
                ...
                <thead>
                  <tr>
                    ...
                    {result.scope_sources.map((source, index) => (
                      <th
                        key={index}
                        className="px-4 py-2 border border-sky-300"
                      >
                        {source}
                      </th>
                    ))}
                  ...
                  {result.similarity_matrix.map((row, i) => (
                    <tr key={i}>
                      ...
                      {row.map((score, j) => (
                        <td
                          key={j}
                          className="px-4 py-2 border border-sky-300 text-center"
                        >
                          {score.toFixed(2)}
                        </td>
                      ))}
              ...
              {result.scopes.map((scope, index) => (
                <div key={index} className="mb-6 bg-sky-800 p-4 rounded-lg">
                  ...
                  {result.spelling_grammar[index] && (
                    <div>
                      <h4 className="text-lg font-medium text-sky-100 mb-2">
                        Issues Found:
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Spelling Issues */}
                        <div>
                          <h5 className="text-sky-100 font-medium mb-2">
                            Spelling:
                          </h5>
                          <ul className="list-disc list-inside text-sky-100">
                            {Object.entries(
                              result.spelling_grammar[index][0]
                        ...
                        <div>
                          ...
                          <ul className="list-disc list-inside text-sky-100">
                            {result.spelling_grammar[index][1].map(
                              (issue, i) => (
